...
echo '<br><br><br><center>STATUS WEBS CLIENTES - '.$fecha.'</center><br>
        <center>
        <table border="1">
            <tr>
...
foreach($webs as $web => $data) {
    ...
    if(!$url || !is_string($url) || !preg_match('/^http(s)?:\/\/[a-z0-9-]+(.[a-z0-9-]+)*(:[0-9]+)?(\/.*)?$/i', $url) && $activo == 1) {    
        ...
        $handle = curl_init($url);
            ...
            if($char_remote == 0) {$diff = 0;} else { $diff = round( ((100 * $char_local) / $char_remote), 1); }
...
echo '     
      ...
      <br><br><center>[ secify.es area de sistemas ]</center>
     ';
